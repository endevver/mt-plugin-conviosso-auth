...
sub invalidate_credentials {
    ...
    my $return_url = sub {
        $app->base . $app->return_uri . $app->make_return_args;
    };
...
} ## end sub invalidate_credentials
...
sub validate_remote_credentials {
    ...
    $ctx->{auth_result}
      = ref $author ? MT::Auth::NEW_LOGIN() : MT::Auth::NEW_USER();
} ## end sub validate_remote_credentials
...
sub validate_local_credentials {
         ...
         or $ctx->{auth_result} == MT::Auth::UNKNOWN() )
    {
        ...
        } ## end if ( defined $username...)

        if ( !$app->user ) {
            $ctx->{auth_result} = $self->redirect_remote_session_test($ctx);
        }
    } ## end if ( $ctx->{auth_result...})
    ...
    return $ctx->{auth_result};
} ## end sub validate_local_credentials
...
sub conviosso_response {
    ...
    $self->verify_query_signature($ctx) or return MT::Auth::UNKNOWN();
...
sub synchronize_user {
    ...
    my $app    = $ctx->{app};
    my $ext_id
      = $ctx->{external_id} ? $ctx->{external_id}
    ...
    my $user_cache = $app->request('convio_user_data') || {};
    return $user_cache->{$ext_id} if $user_cache->{$ext_id};
    ...
    my $mapped_fields = {
            ...
            url => $result->{work_web_page} || $result->{home_web_page} || '',
    };
    ...
    if ( $user and $user->id ) {
        ...
        foreach my $fld ( keys %$mapped_fields ) {
            next
              unless defined $mapped_fields->{$fld}
...
sub remote_login {
    ...
    unless ( defined $ext_id and defined $pass ) {
        return
          $self->error(
              "Missing remote login credential: "
                . join( ', ',
                        grep { defined $ctx->{$_} } qw(external_id password) )
          );
    }
...
sub redirect_single_signon {
    ...
    $app->redirect( $url . $ctx->{convio_api}->client->buildQuery($post) );

    return $ctx->{auth_result} = MT::Auth::REDIRECT_NEEDED();
} ## end sub redirect_single_signon
...
sub redirect_remote_session_test {
    ...
    return $ctx->{auth_result} = MT::Auth::REDIRECT_NEEDED();
} ## end sub redirect_remote_session_test
...
=cut
    ...
    $@
      and
      return $self->error( 'Error creating md5 hash of query string: ' . $@ );
...
=cut
    ...
    return 1;
} ## end sub verify_query_signature
...
sub save_cv_session {
    ...
    $session->set( $_, $ctx->{$_} )
      foreach grep { $_ !~ m{^(convio_api|json_hdlr|app|user)$} } keys %$ctx;
...
sub init_convio {
    ...
    $ctx->{convio_api} = MT::Plugin::ConvioSSOAuth::Util::convio_instance()
